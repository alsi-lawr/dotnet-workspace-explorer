...
module internal WorkspacePageTokens =
    type Payload =
        { WorkspaceId: string
          ParentWorkspaceNodeId: string
          Offset: int
          Revision: int64 }
    ...
    let create (secret: byte array) (payload: Payload) =
        ...
        writeString writer payload.ParentWorkspaceNodeId
                ...
                if not (CryptographicOperations.FixedTimeEquals(expected, supplied)) then
                    ...
                    let payload =
                        { WorkspaceId = readString reader
                          ParentWorkspaceNodeId = readString reader
