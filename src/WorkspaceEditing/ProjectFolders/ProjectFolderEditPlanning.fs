...
module internal ProjectFolderEditPlanning =
    ...
    let private descendantItems (snapshot: ProjectEvaluationSnapshot) projectDirectory folder =
        snapshot.Dimensions
        |> Seq.collect (fun (dimension: ProjectEvaluationDimension) -> dimension.Items)
        |> Seq.choose (fun item ->
            item.ResolvedPath
            |> Option.ofObj
    ...
    let plan
        ...
        if snapshot.CapabilityProfile <> WorkspaceCapabilityProfile.Full then
            ...
            invalid "targetNodeId" "The command target was not found."
            ...
            try
                ...
                let projectDirectory = ProjectItemInclusion.projectDirectory project
                ...
                | "project.folder.rename" ->
                    ...
                    makePlan
                        workspace
                        command
                        [ WorkspaceEditAction.Move(source, destination); save () ]
                        [ projectPath; source; destination; destinationParent destination ]
                | "project.folder.move" ->
                    let source = canonicalDirectory projectDirectory (path "path") |> unwrap
                ...
                | "project.folder.delete" ->
                    ...
                    let actions =
                        if commandId = "project.folder.delete" then
                            [ save (); WorkspaceEditAction.Trash folder ]
                        else
                            [ save () ]
