...
module internal CentralPackageManagement =
    ...
    let private pathComparison workspaceRoot =
        match HostFileSystemCaseDetector.DetectFromExistingPath workspaceRoot with
    ...
    /// canonical package subprocess can mutate a project.  The evaluated snapshot is
    /// authoritative; XML is used only later to render owned replacements.
    let preflight (workspaceRoot: string) (snapshot: EvaluationSnapshot) =
    ...
    let private unsafeNestedOwner (root: string) (project: string) =
        ...
        let rootIdentity = MutationFiles.identity root
        let workspaceIdentity = MutationFiles.identity workspaceRoot
        let mutable current = Path.GetDirectoryName project |> Option.ofObj
        let mutable found = false

        while current.IsSome
              && MutationFiles.identity current.Value <> workspaceIdentity
              && not found do
            let candidate = Path.Combine(current.Value, "Directory.Packages.props")

            if File.Exists candidate && MutationFiles.identity candidate <> rootIdentity then
                found <- true
        ...
        try
            ...
            if unsafeNestedOwner owner project then
                ...
                let projectDocument, projectEncoding, projectPreamble, projectLineEnding =
                    ProjectMutations.readDocument project
                ...
                if List.isEmpty memberships then
                    ...
                    let ownerDocument, ownerEncoding, ownerPreamble, ownerLineEnding =
                        if File.Exists owner then
                            ProjectMutations.readDocument owner
                        else
                            XDocument(
                                XElement(
                                    name "Project",
