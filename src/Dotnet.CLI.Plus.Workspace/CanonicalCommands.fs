...
module internal CanonicalCommands =
    ...
    let discover (workspace: SolutionWorkspace) target =
        ...
        if workspace.WorkspaceDescriptor.IsReadOnly then
            candidates
            |> Seq.filter (fun descriptor -> descriptor.CommandAccess = CommandAccess.Read)
            |> ImmutableArray.CreateRange
        else
            candidates
    ...
    let targetProjectPath workspace request = projectPath workspace request.TargetId
    ...
    let templateOutput workspace request =
        match
            templateDirectory workspace request.TargetId, optionalPath "output" request.Arguments
        with
    ...
    let argv (workspace: SolutionWorkspace) (request: CommandMutationRequest) =
        ...
        | Ok(command, verb), Ok extraArguments ->
            ...
            | "reference" ->
                ...
                match projectPath workspace request.TargetId, frameworkValue, noRestoreValue with
