...
module internal ProjectItemPolicy =
    open ProjectXml
    ...
    let projectDirectory (project: SolutionProjectProjection) =
    ...
    let evaluatedAs (snapshot: EvaluationSnapshot) (itemType: string) (path: string) =
        snapshot.Dimensions
        |> Seq.collect _.Items
        |> Seq.exists (fun item ->
    ...
    let defaultItemType (snapshot: EvaluationSnapshot) (path: string) =
    ...
    let effectiveItemTypes (snapshot: EvaluationSnapshot) includeValue path =
        snapshot.Dimensions
        |> Seq.collect _.Items
        |> Seq.filter (fun item ->
