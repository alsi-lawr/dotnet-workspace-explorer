...
type internal OwnedFileSnapshot =
    private
        { Path: string
          Before: FileImage
          Expected: FileImage option }

type internal OutputEntry = { Path: string; Fingerprint: string }

type internal OutputSnapshot =
    { Root: string
      Existed: bool
      ProjectFiles: Map<string, string> }

module internal CommandCompensation =
    let private projectExtensions = set [ ".csproj"; ".fsproj"; ".vbproj" ]

    let private fileImage path =
        if File.Exists path && not (MutationFiles.isLink path) then
            FileContents(File.ReadAllBytes path)
        elif MutationFiles.exists path then
            NonFile
        else
            Missing
    ...
    let outputSnapshot root =
        ...
        let projects =
            if Directory.Exists root && not (MutationFiles.isLink root) then
                ...
                |> Seq.choose (fun path ->
                    ...
                    if projectExtensions.Contains extension then
                        Some(MutationFiles.identity path, path)
                    else
                        None)
                |> Map.ofSeq
            else
        ...
        { Root = root
          Existed = MutationFiles.exists root
    ...
    let newOutputRoot beforeSnapshot afterSnapshot =
        if beforeSnapshot.Existed || not afterSnapshot.Existed then
            ...
            match MutationFiles.fingerprint afterSnapshot.Root with
            | Ok fingerprint ->
                Some
            ...
            |> Seq.choose (fun snapshot ->
                ...
                | Some _ ->
                    ...
                    | FileContents contents ->
                        Some(MutationAction.ReplaceFile(snapshot.Path, contents))
                    | Missing -> Some(MutationAction.Delete(snapshot.Path, true, false))
                    | NonFile ->
                ...
                { CommandId = commandId
                  ...
                  Intents =
                    ImmutableHashSet.Create(
                        ...
                        MutationIntent.PermanentDelete
                    )
                  AuthorizedRoots =
    ...
    let removeNewOutput entry =
        try
            match MutationFiles.fingerprint entry.Path with
            | Ok fingerprint when fingerprint = entry.Fingerprint ->
                MutationFiles.remove entry.Path

                if MutationFiles.exists entry.Path then
                    Some entry.Path
                else
                    None
