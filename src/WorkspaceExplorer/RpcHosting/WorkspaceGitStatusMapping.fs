...
module internal WorkspaceGitStatusMapping =
    ...
    let mapDecorations workspacePath (nodes: WorkspaceGitNode array) changes =
        let comparison =
            match
                Workspaces.FileSystemCaseSensitivityDetector.DetectFromExistingPath(workspacePath)
            with
            | Workspaces.FileSystemCaseSensitivity.Insensitive -> StringComparison.OrdinalIgnoreCase
        ...
        for state, path in changes do
            for node in nodes do
                let direct =
                    node.PhysicalPath |> Option.exists (fun candidate -> same candidate.Value path)
