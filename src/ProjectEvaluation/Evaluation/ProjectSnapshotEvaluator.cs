...
internal sealed class ProjectSnapshotEvaluator : IDisposable
{
    ...
    internal WorkspaceOutcome<ProjectEvaluationSnapshot> Evaluate(
    ...
    )
    {
        ...
        {
            return ProjectEvaluationOutcomes.NotFound<ProjectEvaluationSnapshot>(
                projectPath,
                ProjectEvaluationDiagnosticCodes.ProjectNotFound,
        ...
        {
            Touch(cached);
            return ProjectEvaluationOutcomes.Success(cached.Snapshot);
        }
        ...
        try
        {
            ...
            return ProjectEvaluationOutcomes.Success(snapshot);
        }
        catch (InvalidProjectFileException)
        {
            return ProjectEvaluationOutcomes.InvalidInput<ProjectEvaluationSnapshot>(
                "projectPath",
                ProjectEvaluationDiagnosticCodes.ProjectMalformed,
        ...
        {
            return ProjectEvaluationOutcomes.Internal<ProjectEvaluationSnapshot>(
                ProjectEvaluationDiagnosticCodes.EvaluationFailed,
    ...
    internal ProjectInvalidationResult Invalidate(
        ImmutableArray<WorkspaceArtifactPath> changedPaths
    )
    {
        ...
        return new ProjectInvalidationResult(invalidated);
    }
    ...
    private Project Load(string projectPath, EvaluatedTargetFramework? targetFramework)
    {
    ...
    {
        var dimensions = ImmutableArray.CreateBuilder<ProjectEvaluationDimension>(projects.Length);
        ...
        var diagnostics = File.Exists(
            ...
            : ImmutableArray.Create(
                ProjectEvaluationOutcomes.Diagnostic(
                    ProjectEvaluationDiagnosticCodes.AssetsMissing,
    ...
    private static ImmutableArray<EvaluatedTargetFramework> GetTargetFrameworks(Project project) =>
        project
            ...
            .Select(framework => new EvaluatedTargetFramework(framework))
            .ToImmutableArray();
    ...
    private static bool Affects(ProjectEvaluationCacheEntry entry, string changedPath)
    {
    ...
    private void Add(
        string projectPath,
        ProjectEvaluationSnapshot snapshot,
        ImmutableArray<Project> projects
    )
    {
        ...
        cache.Add(projectPath, new ProjectEvaluationCacheEntry(snapshot, projects, node));
    }

    private void Touch(ProjectEvaluationCacheEntry entry)
    {
    ...
    private void Unload(ProjectEvaluationCacheEntry entry)
    {
