...
module internal SolutionLaunchProfileCommands =
    ...
    let descriptors =
        ImmutableArray.CreateRange
            [ descriptor "solution.launch-profile.list" "List launch profiles" CommandAccess.Read []
              descriptor
    ...
    let discover (workspace: SolutionWorkspace) (target: WorkspaceNodeId option) =
        if target.IsNone then
            if workspace.Descriptor.IsReadOnly then
                descriptors
                |> Seq.filter (fun item -> item.Access = CommandAccess.Read)
                |> ImmutableArray.CreateRange
            else
                descriptors
        else
    ...
    let argv (workspace: SolutionWorkspace) (request: CommandMutationRequest) =
        match request.CommandId.Value, request.TargetWorkspaceNodeId with
        | "solution.launch-profile.list", None ->
            Ok [ "solution"; workspace.SolutionPath.Value; "launch"; "list" ]
        | "solution.launch-profile.set", None ->
            match
                requiredText "name" request.Arguments, requiredTexts "projects" request.Arguments
            with
            | Ok name, Ok projects ->
                Ok([ "solution"; workspace.SolutionPath.Value; "launch"; "set"; name ] @ projects)
            | Error error, _
            | _, Error error -> Error error
        | "solution.launch-profile.remove", None ->
            requiredText "name" request.Arguments
            |> Result.map (fun name ->
                [ "solution"; workspace.SolutionPath.Value; "launch"; "remove"; name ])
    ...
    let plan (workspace: SolutionWorkspace) (request: CommandMutationRequest) =
        ...
        let prepared =
            match request.TargetWorkspaceNodeId, request.CommandId.Value, name with
            | Some _, _, _ -> Error "The launch-profile command requires a workspace target."
            | None, "solution.launch-profile.set", Ok name ->
                requiredTexts "projects" request.Arguments
                |> Result.bind (SolutionLaunchProfiles.prepareSet workspace name)
            | None, "solution.launch-profile.remove", Ok name ->
                SolutionLaunchProfiles.prepareRemove workspace name
        ...
        |> Result.map (fun (path, contents) ->
            let root =
                IO.Path.GetDirectoryName workspace.SolutionPath.Value
                |> Option.ofObj
            ...
            WorkspaceEditAction.ReplaceFile(path, contents),
    ...
    let verify (workspace: SolutionWorkspace) (request: CommandMutationRequest) =
        ...
        | None, "solution.launch-profile.set", Ok name ->
            requiredTexts "projects" request.Arguments
            |> Result.bind (SolutionLaunchProfiles.verifySet workspace name)
        | None, "solution.launch-profile.remove", Ok name ->
            SolutionLaunchProfiles.verifyRemove workspace name
        | None, _, Error error -> Error error
        | _ -> Error "The launch-profile command is not supported."
